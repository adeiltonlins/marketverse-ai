...
<main class="layout"><section class="war"><div class="grid"></div><div class="center"><div id="core" class="core">🧠</div><h2>COORDENADOR</h2><div id="task" class="task">Aguardando briefing…</div><div id="handoff" class="handoff"></div></div>
...
function showHandoff(agent,preview){const handoff=document.querySelector('#handoff');handoff.innerHTML=`<span class="packet">${agent.icon}</span><span>→ 🧠 Coordenador</span>`;handoff.classList.remove('show');void handoff.offsetWidth;handoff.classList.add('show');setTimeout(()=>{handoff.classList.remove('show');},1800);}
...
function handleAgent(data){const a=agentById(data.id);if(!a)return;const s=states[data.id];s.progress=data.progress??s.progress;s.status=data.status;s.preview=data.preview||s.preview;document.querySelector(`#bar-${a.id}`).style.width=s.progress+'%';document.querySelector(`#status-${a.id}`).textContent=data.message||data.status;const card=document.querySelector(`#agent-${a.id}`);card.classList.toggle('active',data.status==='working');card.classList.toggle('done',data.status==='completed');card.classList.toggle('error',data.status==='error');if(data.preview){document.querySelector(`#result-${a.id}`).innerHTML=`<span>✓ Entrega</span><p>${escapeHtml(data.preview)}</p>`;showHandoff(a,data.preview);}if(data.message)addEvent(a.icon,a.name,data.message);}
